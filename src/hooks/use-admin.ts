...
import { criarUsuarioAdmin, restaurarSenhaUsuario } from "@/services/admin";
...
export function useRestaurarSenhaUsuario() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (data: AdminRestaurarSenhaData) => restaurarSenhaUsuario(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: profileKeys.all });
    },
  });
