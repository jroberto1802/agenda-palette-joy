-- =============================================================================
-- Busca global (projetos, tarefas, subtarefas, avisos, comentários)
-- - SECURITY INVOKER: respeita RLS de cada tabela
-- - fold_search_text: case/accent insensitive
-- =============================================================================

create or replace function public.fold_search_text(t text)
returns text
language sql
immutable
parallel safe
as $$
  select translate(
    lower(coalesce(t, '')),
    'áàâãäåāăąÁÀÂÃÄÅĀĂĄéèêëēĕėęěÉÈÊËĒĔĖĘĚíìîïīĭįıÍÌÎÏĪĬĮóòôõöøōŏőÓÒÔÕÖØŌŎŐúùûüūŭůűųÚÙÛÜŪŬŮŰŲçÇñÑýÿÝ',
    'aaaaaaaaaaaaaaaaaaeeeeeeeeeeeeeeeeiiiiiiiiiiiooooooooooooooooouuuuuuuuuuuuuuuuuccnyyy'
  );
$$;

create or replace function public.escape_like_pattern(t text)
returns text
language sql
immutable
parallel safe
...
begin
  ...
  into v_tarefas
  from (
    select t.id, t.titulo, left(coalesce(t.descricao, ''), 160) as trecho
...
comment on function public.buscar_conteudo(text, integer) is
  'Busca global case/accent-insensitive; respeita RLS (security invoker).';

grant execute on function public.buscar_conteudo(text, integer) to authenticated;
