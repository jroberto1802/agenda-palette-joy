-- =============================================================================
-- Recorrência diária: subtarefa sem deslocamento configurável
-- =============================================================================
-- Na UI (tela Recorrentes), o campo de deslocamento agora fica oculto quando a
-- recorrência da tarefa pai é diária — não há necessidade de configurar dias
-- extras nesse caso. Sem o campo visível, uma subtarefa nova (sem data_inicio
-- legado e sem offset_dias/dia_no_mes) cairia no cálculo implícito legado e
-- poderia ficar sem data em cada ocorrência. Esta migração garante que, para
-- séries diárias, a subtarefa sempre acompanha o mesmo dia da ocorrência pai
-- quando não há um deslocamento explícito configurado.
-- =============================================================================
...
begin
  ...
  loop
    ...
    if v_sub.dia_no_mes is not null then
      ...
      v_data := (v_dia + v_hora) at time zone v_tz;
    elsif coalesce(p_config->>'tipo', 'nenhuma') = 'diaria' then
      v_data := (p_ocorrencia_dia + v_hora) at time zone v_tz;
...
comment on function public.recorrencia_clonar_subtarefas(uuid, uuid, date, date, jsonb) is
  'Clona as subtarefas-template do modelo para uma ocorrência. Prioriza dia_no_mes/offset_dias explícitos; em recorrência diária sem deslocamento usa o mesmo dia da ocorrência; sem nada disso, usa o deslocamento implícito legado a partir de data_inicio.';
...
comment on function public.recorrencia_sincronizar_subtarefas(uuid) is
  'Alinha as subtarefas das ocorrências abertas (hoje em diante) ao template do modelo. Prioriza dia_no_mes/offset_dias explícitos; em recorrência diária sem deslocamento usa o mesmo dia da ocorrência; sem nada disso, usa o deslocamento implícito legado a partir de data_inicio.';
