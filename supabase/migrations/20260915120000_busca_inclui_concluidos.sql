-- =============================================================================
-- Busca global: inclui itens concluídos (baixo contraste na UI)
-- Retorna flag `concluida` para tarefas e subtarefas.
-- =============================================================================
...
begin
  ...
  select coalesce(jsonb_agg(to_jsonb(r) order by r.concluida, r.titulo), '[]'::jsonb)
  into v_tarefas
  from (
    select
      t.id,
...
comment on function public.buscar_conteudo(text, integer) is
  'Busca global case/accent-insensitive (abertos e concluídos); respeita RLS (security invoker).';
